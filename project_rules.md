...
- Non-loopback production requires a 32-byte canonical base64url ingress token supplied through exactly one of server-only `INGRESS_AUTH_TOKEN` or mounted `INGRESS_AUTH_TOKEN_FILE`. Traefik must overwrite inbound `X-Shredit-Ingress-Auth` and `X-Shredit-Surface` from a secret-backed dynamic configuration; never put the token in Docker labels. `TRUSTED_PROXY_CIDRS` identifies controlled upstream forwarding ranges (for example Cloudflare's published CIDRs), not resolved Cloudflare edge addresses or an ephemeral application-container IP.
