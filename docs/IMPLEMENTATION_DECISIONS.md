...
32. Client IP extraction requires an authenticated ingress and a canonical `X-Forwarded-For` chain. `TRUSTED_PROXY_CIDRS` identifies controlled upstream hops such as Cloudflare's published networks, not resolved edge addresses or an ephemeral final Docker peer. In token mode, a chain without at least one trusted upstream hop resolves to `unknown-ip`/unknown country. Rate-limit keys use HMAC-derived IP digests with short TTLs, never raw IP addresses. Unknown GeoIP is treated as non-CN when Turnstile is enabled, so Turnstile is required rather than bypassed.
33. Outside loopback production, the reverse proxy selects the request surface, overwrites `X-Shredit-Surface` with `clearnet` or `onion`, and stamps `X-Shredit-Ingress-Auth` from a server-only secret. The Node runtime compares that value with `INGRESS_AUTH_TOKEN` in constant time. When a token is configured it is mandatory and trusted-peer CIDR fallback cannot bypass it; mutations additionally require an exact configured browser `Origin`. Framework-derived `Host`, forwarded host, and scheme values never select the anti-abuse policy.
...
39. On 2026-09-05 the owner authorized replacing final-container-IP ingress trust with a stable shared ingress-authentication secret. The secret is exactly 32 random bytes encoded as canonical unpadded base64url, is distinct from other application secrets, and must reach Traefik through a secret mount or secret-manager-rendered dynamic configuration rather than a Docker label.

## Open Operator Decisions

The following are intentionally launch-only and must be supplied by the owner: final capacity limits; safe ingress-token provisioning; trusted upstream proxy CIDRs and GeoIP path; onion provisioning and persistent hidden-service key; native EN/zh-CN copy review; final legal pages; and Shredit/Shred-it name/trademark review. They are configuration or publication gates, not reasons to add speculative product behavior.
